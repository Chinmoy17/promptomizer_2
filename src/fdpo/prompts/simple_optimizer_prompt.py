...
_TASK_DESCRIPTIONS = {
    ...
    "ifeval": (
        ...
        "and satisfy ALL stated constraints simultaneously. IMPORTANT: keep "
        "the Constraints/Output Format guidance GENERAL and CONCISE -- do "
        "NOT grow it into an ever-longer per-rule-type checklist (one "
        "paragraph per constraint family you have seen fail). This "
        "benchmark draws from dozens of constraint types, so a checklist "
        "can never cover them all, and a long checklist forces the solver "
        "to spend its token budget on preflight bookkeeping before it "
        "reaches the graded content -- which can truncate the actual "
        "answer entirely on longer tasks. Prefer one short, general "
        "instruction (identify every constraint stated in THIS request, "
        "then verify the final content satisfies each one before "
        "answering) over enumerating specific rule families."
    ),
    "ifbench": (
