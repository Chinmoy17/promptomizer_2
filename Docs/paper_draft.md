...
| AIME (2022-24 → 2025) | competition math, integer answer | `reflect_fdpo` | 90 → 58/32 | 30 | GPT-4o-mini, Claude Haiku 4.5, GPT-4.1, GPT-4.1 Mini | GPT-5 |
...
| AIME (2022-24→2025) | GEPA baseline 27.33 (Qwen3-8B) / 49.33 (GPT-4.1 Mini); GEPA-optimized 32.00 / 59.33 | `reflect_fdpo`: Claude Haiku 4.5 0.267→0.333 (+6.7pp, validation stayed ≥baseline every round); GPT-4o-mini and GPT-4.1 both reverted; GPT-4.1 Mini 0.333→0.333 (net test churn +0: 4 recovered/4 regressed — a wash, not a regression) | **Verified from the GEPA paper directly:** their baseline is a DSPy `ChainOfThought`-scaffolded system, not a bare instruction. Our baseline is a deliberately bare, vague seed. This — not solver capability alone — plausibly explains most of the baseline gap; only within-method deltas are informative here. The GPT-4.1 Mini run also predates the mechanism fixes applied for IFBench/PUPA (stale "last round ships" instruction), and its shipped round won only via a validation-accuracy tie broken by commit order — the untied alternative round scored notably better on mining |
| PUPA | GEPA: 78.57→**94.47/96.46** (GPT-4.1 Mini) | `reflect_fdpo`: mean composite score 0.685→0.799 (GPT-4o-mini, +11.4pp), 0.805→0.843 (Claude Haiku 4.5, +3.8pp, zero test-set regressions), 0.682→**0.807** (GPT-4.1 Mini, +12.5pp; accuracy 0.447→0.711; net test churn +10, zero regressions) | The one benchmark where our scoring formula — (quality + (1−leakage))/2 — is implemented identically to the source paper's construction, so this comparison is on the same measurement scale by construction, not merely by report. GPT-4.1 Mini is also the first case in this paper where our solver model is literally the same model GEPA reports; this result required fixing a hardcoded judge-token truncation bug (the quality judge, a reasoning model, could be cut off before writing its score) and the same stale "last round ships" optimizer instruction fixed for IFBench — the GPT-4o-mini and Claude Haiku 4.5 numbers predate both fixes and are not directly comparable to it. Even after the fix, our baseline composite (0.682) sits far below GEPA's reported GPT-4.1 Mini baseline (0.786), most plausibly from split-size/composition differences (60/40 pool vs. the official 111/111/221) or pipeline construction, not solver capability, so even this comparison is not head-to-head |

Three solver models on PUPA is itself informative, with a caveat: GPT-4.1 Mini's result below was
measured after fixing a judge-token-truncation bug and a stale optimizer-mechanism instruction,
while the GPT-4o-mini and Claude Haiku 4.5 numbers were not remeasured under those same fixes, so
the three are not a clean like-for-like comparison. Claude Haiku 4.5's baseline is already much
stronger out-of-the-box (composite 0.805 vs. GPT-4o-mini's 0.685 vs. GPT-4.1 Mini's 0.682),
...
recovered, zero regressed). GPT-4.1 Mini now shows the largest and cleanest optimized gain of the
three (+12.5pp composite; net test churn +10, i.e. 10 recovered and zero regressed) — but given
it is also the only one of the three measured post-fix, this should be read as "the mechanism can
produce a large, clean gain on this task once these bugs are fixed," not as evidence that GPT-4.1
Mini is the strongest solver for PUPA specifically.
